...
import 'my_job_list_model.dart';
export 'my_job_list_model.dart';
...
class MyJobListScreenWidget extends StatefulWidget {
  const MyJobListScreenWidget({Key? key}) : super(key: key);

  @override
  _MyJobListScreenWidgetState createState() =>
      _MyJobListScreenWidgetState();
}

class _MyJobListScreenWidgetState
    extends State<MyJobListScreenWidget> {
  late MyJobListScreenModel _model;
  ...
  void initState() {
    getJobList();
    super.initState();
    _model = createModel(context, () => MyJobListScreenModel());
    init();
  }
  ...
  getJobList() async {
    ...
    try {
      //var jobQuery = await db.collection('/job').get();
      var jobQuery = await db.collection('/job').where('status', isEqualTo: true).get();
      ...
      for (var jobDoc in jobQuery.docs) {
        var participantQuery = await jobDoc.reference.collection('applicants').where('uid', isEqualTo: user_uid).get();

        if (participantQuery.docs.isNotEmpty) {
          // If the participant with user_uid is found in the subcollection
          Jobs? temp = Jobs.fromDocument(jobDoc);
          jobList.add(temp);
        }
      }
